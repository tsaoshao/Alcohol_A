...
Comment <- read_csv("/Users/xiaoya/Desktop/UvA/R/AlcoholData/Comment.csv")
DayData <- read_csv("/Users/xiaoya/Desktop/UvA/R/AlcoholData/DayData.csv")
Like <- read_csv("/Users/xiaoya/Desktop/UvA/R/AlcoholData/Like.csv")
Login <- read_csv("/Users/xiaoya/Desktop/UvA/R/AlcoholData/Login.csv")
Post <- read_csv("/Users/xiaoya/Desktop/UvA/R/AlcoholData/Post.csv")
PreSurvey <- read_csv("/Users/xiaoya/Desktop/UvA/R/AlcoholData/Presurvey.csv")
User <- read_csv("/Users/xiaoya/Desktop/UvA/R/AlcoholData/User.csv")
WeekData <- read_csv("/Users/xiaoya/Desktop/UvA/R/AlcoholData/WeekData.csv")
```
...
**Check primary key for user_sub_1 (Group) table**
...
In the initial `Group` table, there was one row with `Group_nr = NA` and `Condition = NA`.\
Since `Group_nr` is the primary key of the `Group` table, it must not contain missing values.\
The NA row does not correspond to a real group (it originates from users without group membership), so it was removed from the `Group` table.

**Remove missing value from user_sub_1 (Group) table**
...
**Check primary key for user_sub_2 (User) table**
...
`Group_nr` is a foreign key that links participants to groups.\
Missing values in `Group_nr` and `ID_nr_Vragenlijst` occur only for `admin` and `special` users, who did not complete the questionnaire and are not assigned to any group. Therefore, these NAs are allowed and are not treated as data errors.
...
3.2.3 Group membership foreign key
...
+-------------------+----------------------------------+---------------------------------+
| Tables            | Primary Keys                     | Foreign Keys –\> Table          |
+===================+==================================+=================================+
...
2\. The Groups table contains a unique row per experimental group and is identified by the primary key Group_nr. Since this table does not reference any other entity, it does not contain foreign keys.
...
left_join(User_sub1, User_sub2, by = "Group_nr") |>
  count(Group_nr) |> summarise(max_n = max(n))
```

After joining `User_sub1` (Group table) with `User_sub2` (User table) by `Group_nr`, the largest number of users belonging to the same group is **20 ,** which means each `Group_nr` in `User_sub1` corresponds to **multiple** rows in `User_sub2`. Therefore, the relationship between `User_sub1` and `User_sub2` is **one-to-many.**

```{r}
left_join(Login, User_sub2, by = "UserID") |>
  count(UserID) |> summarise(max_n = max(n))
```

After joining **User_sub2 (User table)** with **Login (login records)** by `UserID`, the largest number of login records associated with a single user is **158**, which means at least one user appears **158 times** in the joined table. This indicates that:

-   each `UserID` in **User_sub2** corresponds to **multiple rows** in **Login**

-   i.e., a single user has **many** login events.

Therefore, the relationship between **User_sub2** and **Login** is **one-to-many.**

```{r}
left_join(Post_sub1, Comment_tidy, by = "PostID") |>
  count(PostID) |> summarise(max_n = max(n))
```

After joining **Post_sub1 (Post table)** with **Comment_tidy (Comment table)** by `PostID`, the largest number of comment records associated with a single post is **10**, which means at least one post appears **10 times** in the joined table.

This indicates that:

-   each `PostID` in **Post_sub1** corresponds to **multiple rows** in **Comment_tidy**,

-   i.e., a single post has **many** comments.

Therefore, the relationship between **Post_sub1** and **Comment_tidy** is **one-to-many**.

```{r}
left_join(User_sub2, Comment_tidy, 
                    by = c("UserID" = "CommenterID")) |> 
  count(UserID) |> 
  summarise(max_n = max(n))
```

After joining **User_sub2 (User table)** with **Comment_tidy (Comment table)** by matching `UserID` with `CommenterID`, the largest number of comments written by a single user is **34**,\
which means that at least one user appears **34 times** in the joined table.

This indicates that:

-   each `UserID` in **User_sub2** corresponds to **multiple rows** in **Comment_tidy**,

-   that is, a single user can write **many** comments.

Therefore, the relationship between **User_sub2** and **Comment_tidy** is **one-to-many**
...
After joining **User_sub2 (User table)** with **DayData_tidy (Daily data table)** by `UserID`, the largest number of daily records associated with a single user is **66**, which means that at least one user appears **66 times** in the joined table.

This indicates that:

-   each `UserID` in **User_sub2** corresponds to **multiple rows** in **DayData_tidy**,

-   that is, a single user can have **many** daily records.

Therefore, the relationship between **User_sub2** and **DayData_tidy** is **one-to-many.**
...
After joining **Post_sub1 (Post table)** with **Like_tidy (Like table)** by `PostID`, the largest number of likes associated with a single post is **122**, which means that at least one post appears **122 times** in the joined table. This indicates that a single post can receive **many likes**, so the relationship between **Post_sub1** and **Like_tidy** is **one-to-many.**
...
After joining **User_sub2 (User table)** with **Like_tidy (Like table)** by matching `UserID` with `LikerID`, the largest number of likes given by a single user is **196**, which means that at least one user appears **196 times** in the joined table. This indicates that a single user can give **many likes**, so the relationship between **User_sub2** and **Like_tidy** is **one-to-many**
...
After joining **User_sub2 (User table)** with **Post_sub2 (View table)** by matching `UserID` with `ViewerID` the largest number of views made by a single user is **487**, which means that at least one user appears **487 times** in the joined table. This indicates that a single user can view **many posts**, so the relationship between **User_sub2** and **Post_sub2** is **one-to-many**
...
After joining **User_sub2 (User table)** with **PreSurvey_sub1 (Friend nomination table)** by `UserID`, the largest number of nominated friends associated with a single user is **20**, which means that at least one user appears **20 times** in the joined table. This indicates that a single user can nominate **many members**, so the relationship between **User_sub2** and **PreSurvey_sub1** is **one-to-many**
...
After joining **User_sub2 (User table)** with **PreSurvey_sub2 (One-row-per-user survey)**\
by `UserID`, the largest number of survey rows associated with a single user is **1**, which means every user appears **at most once** in the joined table. This indicates a **one-to-one** relationship between **User_sub2** and **PreSurvey_sub2**
...
After joining **User_sub2 (User table)** with **PreSurvey_sub2 (One-row-per-user survey)** by `UserID`, the largest number of survey rows associated with a single user is **1**, which means every user appears **at most once** in the joined table. This indicates a **one-to-one** relationship between **User_sub2** and **PreSurvey_sub2**
